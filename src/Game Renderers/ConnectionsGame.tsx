...
function ConnectionsGame({ gameInfo, onGameEnd, multiplayer }: ConnectionsGameProps) {
  ...
  const revealRemaining = (currentSolved: SolvedGroup[]) => {
    ...
    later(() => endGame(currentSolved.length), 300 + remaining.length * 260 + 500);
  };
  ...
  const handleSubmit = () => {
    ...
    if (matchIdx >= 0) {
      ...
      if (nextSolved.length === 4) {
        ...
        flash(`Correct! +${POINTS_PER_GROUP}`, "var(--good)");
      }
      return;
    }
  ...
  if (!board.tiles || board.tiles.length !== 16 || !board.groups || board.groups.length !== 4) {
    return <p className="cn-note">This board is unavailable. Please try another round.</p>;
  }
  ...
  return (
    <GameFrame>
      <GameFrame.Status
        left={<GameFrame.Label>FIND FOUR GROUPS</GameFrame.Label>}
        right={
          <span className="cn-hearts" aria-label={`${hearts} of ${START_HEARTS} hearts left`}>
              ...
              <AnimatePresence>
                {solved.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <span className="cn-solved-label font-display">{s.label}</span>
